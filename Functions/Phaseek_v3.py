...
class HeadMixture(nn.Module):
    def __init__(self, m: int, n_heads: int, tau: float = 1.5, l2_delta: float = 1e-4):
        ...
        self.delta = nn.Parameter(torch.zeros(n_heads, m))  
        self.l2_delta = l2_delta
    def forward(self, Lhat_stack): 
        B, m, T, _ = Lhat_stack.shape
        logits = self.alpha.unsqueeze(0) + self.delta
        pi = torch.softmax(logits / self.tau, dim=-1)      
        pi = pi.to(Lhat_stack.dtype, copy=False).to(Lhat_stack.device, non_blocking=True)
        Bh = torch.einsum('hm,bmij->hbij', pi, Lhat_stack)  
        Bh = Bh.permute(1, 0, 2, 3).contiguous()             
...
class FEGSTrans(nn.Module):
    ...
    def _masked_zscore(self, bm, key_mask):
        ...
        if H > 1: valid = valid.expand(-1, H, -1, -1)
        ...
        var  = torch.where(valid, (bm - mean) ** 2, torch.zeros_like(bm)).sum(dim=(-2, -1), keepdim=True) / count
...
class Block(nn.Module):
    ...
    def forward(self, x, bias_per_head=None, key_padding_mask=None):
        x = x + self.attn(self.ln_1(x), bias_per_head=bias_per_head, key_padding_mask=key_padding_mask)
...
class TransformerClassifier(nn.Module):
    def __init__(self, config: Config, topk_m: int, label_smooth: float = 0.05, weight_decay: float = 0.1):
        super().__init__()
        self.config = config
    ...
    def forward(self, idx, targets=None, Lhat_stack=None):
        B, T = idx.size()
        ...
        lens  = valid.sum(dim=1).clamp_min(1.0)
        pooled = x_sum / lens

        logits = self.head(pooled)
        loss = None
        if targets is not None:
            loss = F.cross_entropy(logits, targets, label_smoothing=self.label_smooth) + reg_mixture * 1.0
        return logits, loss
